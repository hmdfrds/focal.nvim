...
-- Helper to reset all focal modules between tests
local function reset_focal()
    for key in pairs(package.loaded) do
        if key == "focal" or key:find("^focal%.") then
...
local function make_test_env(opts)
    ...
    local Renderer = require("focal.renderer")
    local Window = require("focal.window")
    local Preview = require("focal.preview")
...
T["setup() with invalid config warns and uses defaults"] = function()
    ...
    require("focal").setup({ min_width = "banana" })
...
T["enable/disable toggle works"] = function()
    ...
    focal.setup({})
    MiniTest.expect.equality(focal.is_enabled(), true)
    focal.disable()
    MiniTest.expect.equality(focal.is_enabled(), false)
    focal.enable()
    MiniTest.expect.equality(focal.is_enabled(), true)
end
...
T["custom source registered before setup survives"] = function()
    reset_focal()
    local focal = require("focal")
    focal.register_source(H.mock_source({ filetype = "custom_pre_setup" }))
    focal.setup({})
    local Resolver = require("focal.resolver")
...
T["focal.show(path) works from public API"] = function()
    reset_focal()
    local focal = require("focal")
    focal.setup({})
