---@mod iris.renderers.image "image.nvim Renderer"
---@brief [[
...
function M.is_available()
    local term = require("iris.terminal").detect()
...
function M.render(ctx, done)
    ...
    if ok then
        ...
        vim.notify("[iris] image.nvim render failed: " .. tostring(err), vim.log.levels.DEBUG)
        done(false)
    end
end
