---@mod iris.config "Configuration"
---@brief [[
--- Config schema, validation, and merging for iris.nvim.
---@brief ]]

---@class IrisChafaConfig
---@field format string
...
---@field backend IrisBackend?
---@field chafa IrisChafaConfig
...
local function validate_min_max(cfg, min_key, max_key)
    if cfg[min_key] and cfg[max_key] and cfg[min_key] > cfg[max_key] then
        warn(
            string.format(
                "[iris] config.%s (%d) > config.%s (%d), swapping",
                min_key,
                cfg[min_key],
                max_key,
                cfg[max_key]
...
--- Merge user options with defaults, validate, and return a complete IrisConfig.
    ...
    for key, _ in pairs(user_opts) do
        if expected_types[key] == nil then
            ...
            if suggestion then
                warn(
                    string.format(
                        "[iris] Unknown config key '%s'. Did you mean '%s'?",
                        key,
                        suggestion
                    )
                )
                ...
                warn(
                    string.format("[iris] Unknown config key '%s'.", key)
                )
            end
        end
    end
